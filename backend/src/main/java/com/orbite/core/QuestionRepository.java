...
class QuestionRepository {
  ...
  List<QuestionDto> findQuestions(String track, String category, String topic, String difficulty) {
    StringBuilder sql = new StringBuilder("select id, prompt, category, topic, track, difficulty, expected_answer_seconds, explanation, example_answer, common_mistakes, problem_statement, test_cases, starter_code, solution_code from public.questions where active = true");
  ...
  private QuestionDto question(ResultSet rs) throws SQLException {
    ...
    return new QuestionDto(id, rs.getString("prompt"), rs.getString("category"), rs.getString("topic"), rs.getString("track"), rs.getString("difficulty"), rs.getInt("expected_answer_seconds"), rs.getString("explanation"), rs.getString("example_answer"), rs.getString("common_mistakes"), rs.getString("problem_statement"), rs.getString("test_cases"), rs.getString("starter_code"), rs.getString("solution_code"), followUps);
  }
