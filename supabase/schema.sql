...
-- Compatibility migration for the adaptive interview service.
alter table if exists public.interview_sessions add column if not exists experience_level text;
alter table if exists public.interview_sessions add column if not exists target_role text;
alter table if exists public.interview_sessions add column if not exists interview_type text;
alter table if exists public.interview_sessions add column if not exists duration_minutes integer;
alter table if exists public.interview_sessions add column if not exists consent_confirmed_at timestamptz;
alter table if exists public.interview_sessions add column if not exists track text;
alter table if exists public.interview_sessions add column if not exists difficulty text;
alter table if exists public.session_answers add column if not exists question_id uuid;
alter table if exists public.session_answers add column if not exists answer_kind text not null default 'PRIMARY';
alter table if exists public.session_answers add column if not exists paste_detected boolean not null default false;
alter table if exists public.session_answers add column if not exists paste_event_count integer not null default 0;
alter table if exists public.session_answers add column if not exists pasted_characters integer not null default 0;
alter table if exists public.questions add column if not exists category text;
alter table if exists public.questions add column if not exists topic text;
alter table if exists public.questions add column if not exists expected_answer_seconds integer default 90;
alter table if exists public.questions add column if not exists explanation text;
alter table if exists public.questions add column if not exists example_answer text;
alter table if exists public.questions add column if not exists common_mistakes text;
create table if not exists public.question_follow_ups (
  ...
  step_order integer not null, stage text not null, prompt text not null
);
create table if not exists public.interview_reports (
  ...
  overall_score integer not null, technical_knowledge integer not null, problem_solving integer not null,
  communication integer not null, system_design integer not null, follow_up_handling integer not null,
  strengths text not null, improvements text not null, recommended_practice text not null, communication_coach text not null
);
alter table if exists public.questions drop constraint if exists questions_track_check;
