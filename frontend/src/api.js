...
export const api = {
  ...
  nextInterviewTurn: (id, body) => request(`/api/interviews/${id}/next-turn`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  getPerformanceOverview: range => request(`/api/performance/overview?range=${encodeURIComponent(range || 'ALL')}`),
  getPerformanceSkills: () => request('/api/performance/skills'),
  getInterviewHistory: range => request(`/api/interviews/history?range=${encodeURIComponent(range || 'ALL')}`),
  getInterviewResults: id => request(`/api/interviews/${id}/results`),
  getRecommendedPractice: category => request(`/api/practice/recommended${category ? `?category=${encodeURIComponent(category)}` : ''}`),
  getPracticeAttempts: () => request('/api/practice/attempts'),
  startPracticeAttempt: problemId => request(`/api/practice/${problemId}/attempt`, { method: 'POST' }),
  completePracticeAttempt: (attemptId, body) => request(`/api/practice/attempts/${attemptId}`, { method: 'PATCH', body: JSON.stringify(body) }),
  runPracticeCode: body => request('/api/practice/run', { method: 'POST', body: JSON.stringify(body) }),
